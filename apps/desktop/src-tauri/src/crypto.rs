...
/// Result of encrypting a recovery packet.
#[derive(Serialize)]
pub struct RecoveryPacketResult {
    pub nonce: Vec<u8>,
...
pub fn decrypt_item(
    ...
    let item_key_aad = format!("zero-vault:item-key-wrap:{}", item_id);
    let item_key = crypto_core::decrypt_xchacha20(
        &vault_key,
        &item_key_blob,
        item_key_aad.as_bytes(),
    )
...
///
/// Returns `{ nonce: Vec<u8>, ciphertext: Vec<u8> }`.
/// The nonce (24 bytes) is randomly generated by crypto-core.
#[tauri::command]
pub fn encrypt_recovery_packet(
    recovery_key: Vec<u8>,
    vault_key: Vec<u8>,
) -> Result<RecoveryPacketResult, String> {
    let blob = crypto_core::encrypt_recovery_packet(&recovery_key, &vault_key)
        .map_err(|e| format!("failed to encrypt recovery packet: {e}"))?;
    Ok(RecoveryPacketResult {
        nonce: blob.nonce.to_vec(),
        ciphertext: blob.ciphertext,
    })
}

/// Decrypt a recovery packet using XChaCha20-Poly1305.
///
/// Expects the recovery key, nonce, and ciphertext (as separate vectors).
/// Returns the vault key bytes (32 bytes) on success.
#[tauri::command]
pub fn decrypt_recovery_packet(
    ...
    if nonce.len() != crypto_core::XCHACHA20_NONCE_LEN {
        return Err("invalid nonce length".to_string());
    }
    let mut nonce_arr = [0_u8; crypto_core::XCHACHA20_NONCE_LEN];
    nonce_arr.copy_from_slice(&nonce);
    let blob = crypto_core::EncryptedBlob {
        nonce: nonce_arr,
        ciphertext,
    };
    crypto_core::decrypt_recovery_packet(&recovery_key, &blob)
        .map_err(|_| "failed to decrypt recovery packet".to_string())
}
