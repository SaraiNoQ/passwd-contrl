...
export interface OfflineSyncState {
  /** Whether the browser/Tauri webview reports being online. */
  isOnline: boolean;
...
export function useOfflineSync(syncFn: () => Promise<void>): OfflineSyncState {
  ...
  const refreshCounts = useCallback(() => {
    ...
    setPendingCount(active.filter((e) => e.retryCount < MAX_RETRIES).length);
    setFailedCount(active.filter((e) => e.retryCount >= MAX_RETRIES).length);
  }, []);
